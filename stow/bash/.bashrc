...
alias yay='yay --answeredit=None --answerdiff=None --noremovemake'
alias ffind='find . -iname'
alias ipv4='ip a | grep "inet " | grep ".*127\.0\.0\.1.*" -v | sed "s/\/19.*//" | sed "s/.*inet //"'
...
if [[ -f ~/THIS_DEVICE ]] && [[ "$(cat ~/THIS_DEVICE)" == "laptop" ]]; then
  ...
  alias osy="pushd ~/repos/StudyingSpace; bash simplesync.sh; popd"
